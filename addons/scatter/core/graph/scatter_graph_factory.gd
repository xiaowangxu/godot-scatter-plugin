...
static func create_default() -> ScatterGraph:
	...
	var group := ScatterNodeRegistry.create_node(&"group")
	var final_output := ScatterNodeRegistry.create_node(&"final_output")
	for item in [
		[box, Vector2(40, 20)],
		...
		[group, Vector2(930, 160)],
		[final_output, Vector2(1230, 160)],
	]:
		if item[0] != null:
			graph.add_node(item[0], item[1])
	if null in [box, random, random_rotation, scale, group, final_output]:
		return graph
	...
	graph.connect_nodes(scale.node_id, &"instances", group.node_id, &"placement")
	graph.connect_nodes(group.node_id, &"set", final_output.node_id, &"sets", 0)
	return graph
