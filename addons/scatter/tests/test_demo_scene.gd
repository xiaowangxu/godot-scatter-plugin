...
func _init() -> void:
	...
	assert(config != null and config.version == 2)
	assert(config.connections.size() == 6)
	var paint_nodes := 0
	var output_nodes := 0
	for entry in config.nodes:
		paint_nodes += 1 if entry.get("type", "") == "paint_region" else 0
		output_nodes += 1 if entry.get("type", "") == "output" else 0
	assert(paint_nodes == 1 and output_nodes == 1)
	var output := config.output_node()
	assert(not config.incoming_connection(output.id, 0).is_empty())
	assert(not config.incoming_connection(output.id, 1).is_empty())
