...
Box 与 Sphere 使用确定性直接采样；Boolean Shape 在组合 AABB 内统一拒绝采样，因此 Union 按体积保持均匀密度。Poisson 使用真正的确定性 3D Bridson 算法。Path 按总弧长采样，Path Tube Region 可把一维 Path 转换为三维 Region。Shape Transform 可在 MultiMesh Local 空间中组合变换 Shape 或 Path。
