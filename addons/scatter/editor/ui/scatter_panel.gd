...
func set_undo_redo(value: EditorUndoRedoManager) -> void:
	...
	if _recipe_links != null:
		_recipe_links.undo_redo = value


func set_recipe_link_controller(value: ScatterRecipeLinkController) -> void:
	if _recipe_links != null and _recipe_links.changed.is_connected(_attachment_changed):
		_recipe_links.changed.disconnect(_attachment_changed)
	_recipe_links = value
	if _recipe_links != null and not _recipe_links.changed.is_connected(_attachment_changed):
		_recipe_links.changed.connect(_attachment_changed)
...
func _commit_graph_property(property: StringName, value: Variant, caption: String, merge := UndoRedo.MERGE_DISABLE) -> void:
	if graph == null or graph.get(property) == value:
		return
	if _graph_editor != null and _graph_editor.editor_context != null:
		_graph_editor.editor_context.commit_property(graph, property, value, caption, "", merge)
...
func _clear_active_paint() -> void:
	...
	var undo := ScatterUndoService.new(_undo_redo, target, notify_viewport_data_changed)
	undo.commit_property(paint_node, &"strokes", empty, tr("Clear Paint Layer"))


func notify_viewport_data_changed() -> void:
	if _graph_editor != null and _graph_editor.editor_context != null:
		_graph_editor.editor_context.notify_model_changed()
	if is_instance_valid(target):
...
func _link_recipe_with_undo(value: ScatterGraph, caption: String) -> void:
	if value == null or not is_instance_valid(target):
		return
	if _recipe_links == null:
		set_recipe_link_controller(ScatterRecipeLinkController.new(_undo_redo))
	_recipe_links.link(target, value, caption)


func _attachment_changed(owner: MultiMeshInstance3D) -> void:
