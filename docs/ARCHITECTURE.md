...
| `plugin.gd` 直接扫描依赖、递归 Build、处理结果并写 MultiMesh | 生命周期入口承担业务流程，无法替换执行策略 | `ScatterBuildCoordinator` 和 Scheduler/Backend 接管 |
...
| 不同 Proxy 图的 output count 共用 `node_id:port` | 相同 node ID 会互相覆盖 | 按 execution / graph / target 分域 |
...
- 只在一次根 Build 中复用中间输出；
- Proxy 递归共享同一个 execution ID，因此同一依赖在一次 Build 中可以复用；
...
5. Proxy 递归必须沿用同一个 Generation backend 和 EvaluationSession，才能维持循环检测、诊断和缓存域。
...
- 每个 Build 中，每个计划节点最多执行一次；递归 Proxy 使用同一会话检测循环。
